...
```
For the third moment, we $log$ the response variable as well so that it is scaled it properly to the same 
```{r mom4 final, eval = F}
set.seed(1)

data_ctrl <- trainControl(method = "cv", number = 5)
model_caret <- train(log(R_moment_4) ~ St + Fr.logit + Re + St*Fr.logit + St*Re + Fr.logit*Re, data=train1,                       
                     trControl = data_ctrl,              # folds
                     method = "lm",                      # specifying regression model
                     na.action = na.pass)
